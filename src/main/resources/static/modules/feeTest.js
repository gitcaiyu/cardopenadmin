define(['layui', 'text!../../pages/feeTest.html'], function (layui, feeTest) {
    var form = layui.form;
    ...
    var obj = {
        ...
        data: function () {
            return {
                ...
                feecalcuTypelistShow:false,/**计算类型未选择**/
                feeSolidShow:false,/**固定金额**/
                ...
                feeEditItem:{},
                isRequired:false,//判断其他的计算类型是否必输
                is_number:false,
        ...
        mounted: function () {
            ...
            initOtherCondition=$.extend(true,{},_this.feeSetItem.other_condition);
            initInnerData=$.extend(true,{},_this.feeCondition_curr);
            initFeeSetData=$.extend(true,{},_this.feeSetItem);
            console.log(initOtherCondition,initInnerData,initFeeSetData)
            _this.getPage({
                curr: _this.curr,
                limit: _this.limit,
                keyword: _this.keyword
        ...
        methods: {
            ...
            checkOuterRule:function(){
                var _this=this;
                $('#other_feeFixAmt').attr('lay-verify','');
                $('#other_feePct').attr('lay-verify','');
                $('#other_feePctBase').attr('lay-verify','');
                
                if(this.feeSetItem.other_condition.feeActionTyp=='NOAMT'){
                    $('#other_feeFixAmt').attr('lay-verify','required|number');
                }else if(this.feeSetItem.other_condition.feeActionTyp=='PCTAMTALL'){
                    $('#other_feePct').attr('lay-verify','required|checkNum|number');
                    $('#other_feePctBase').attr('lay-verify','required');
                };
                form.render('','otherCondition');
                /**选择“收费条件”**/
                 form.on('radio(feeCondOpt)', function (data) {
                    _this.feeSetItem.feeCondOpt=data.value;
                    $('#other_feeFixAmt').attr('lay-verify','');
                    $('#other_feePct').attr('lay-verify','');
                    $('#other_feePctBase').attr('lay-verify','');
                    _this.initRadioSelect({value:"",name:'feeResource'});
                    form.render('','otherCondition');
                    _this.isRequired=true;
                    if(data.value == 1){
                        _this.feeConditionList=true;
                        _this.feeConditionAdd=true;
                        form.render('','feeConditionAdd');
                    }else if(data.value == 2 || data.value == 3 ){
                        _this.feeConditionList=false;
                        _this.initRadioSelect({value:"",name:'calcuType'});
                        $.extend(true,_this.feeSetItem.other_condition,initOtherCondition);
                        form.render('','otherCondition');
                    };
                    if(!_this.feecalcuType){
                        _this.feecalcuTypelistShow=false;
                    }

                });
                 /**选择计算类型**/
                form.on('radio(feeActionTyp)', function (data) {
                    _this.isRequired=false;
                    _this.feeSetItem.other_condition.feeFixAmt="";
                    _this.feeSetItem.other_condition.feePctBase="";
                    _this.feeSetItem.other_condition.feePct="";
                    _this.feeSetItem.other_condition.feeActionTyp=data.value;
                    _this.feecalcuTypelistShow=true;
                    if(data.value == 'NOAMT'){
                        _this.feeSolidShow=true;
                        $('#other_feeFixAmt').attr('lay-verify','required|number');
                        $('#other_feePct').attr('lay-verify','');
                        $('#other_feePctBase').attr('lay-verify','');
                    }else if(data.value == 'PCTAMTALL'){
                        _this.feeSolidShow=false;
                        $('#other_feeFixAmt').attr('lay-verify','');
                        $('#other_feePct').attr('lay-verify','required|checkNum|number');
                        $('#other_feePctBase').attr('lay-verify','required');
                    };
                    form.render('','feeActionTyp_list');
                });
                form.render('','otherCondition');
            },
            /***初始化多条新增**/
            ...
            /**判断多条之外的数据**/
            checkOtherRule:function(){
                var _this=this;
                if(this.feeSetItem.feeCondOpt!=1){
                    console.log(this.feeSetItem.feeActionTyp)
                    if(!this.feeSetItem.feeActionTyp){
                        layer.msg('请选择费用计算类型',{icon:7});
            ...
            fcRequired:function(obj){
               ...
               if(!obj){
                ...
                var reg=/^(-?\d+)(\.\d+)?$/;
                reg.test(obj.feeMinTnr.value)?this.is_feeMinTnr=true:this.is_feeMinTnr=false;
                reg.test(obj.feeMaxTnr.value)?this.is_feeMaxTnr=true:this.is_feeMaxTnr=false;
                ...
                reg.test(obj.feePct.value)?this.is_feePct=false:this.is_feePct=true;
            ...
            getRidioSelectVal:function(param){
                var _this=this;
                var item_other=this.feeSetItem.other_condition;
                form.on('select('+param.filter+')',function(data){
                    if(item_other.hasOwnProperty(param.filter.slice(6))){
                        item_other[param.filter.slice(6)]=data.value;//去掉other_
                    }else{
            ...
            initAdd:function(){
                ...
                this.feecalcuType="";/**计算类型**/
                this.feecalcuTypelistShow=false;/**计算类型未选择**/
                this.feeSolidShow=false;/**固定金额**/
                this.feeCondition_list=[];
                $.extend(true,this.feeSetItem,initFeeSetData);
            ...
            saveInfo:function(msg){
                ...
                _this.getRidioSelectVal({filter:'other_feeResource'});
                _this.getRidioSelectVal({filter:'other_feePctBase'});
                form.on('submit(add)', function (data) {
                    console.log(_this.feeSetItem.feeCondOpt)
                    if(_this.feeSetItem.feeCondOpt==1){
                        ...
                        _this.feeSetItem.pFeeDtlList=[];
                    };
                    _this.fcAddAjax(_this.feeSetItem,_this.feeSetItem.other_condition);
                    _this.checkOtherRule();
                    if(_this.isRequired && _this.feeSetItem.feeCondOpt!=1)return false;
                    $.ajax({
                ...
                $.ajax({
                    ...
                    success: function (result) {
                        ...
                        if(result.fee.feeCondOpt==1){
                            ...
                            _this.feeConditionList=false;
                            _this.feecalcuTypelistShow=true;
                            _this.initRadioSelect({value:item.feeResource,name:'feeResource'});
                            _this.initRadioSelect({value:item.feeActionTyp,name:'calcuType'});
                            if(item.feeActionTyp=='NOAMT'){
                                _this.feeSolidShow=true;
                            }else{
                                _this.feeSolidShow=false;
                                _this.initRadioSelect({value:item.feePctBase,name:'feePctBase'});
                            };
                            for(var a in _this.feeSetItem.other_condition){
                                _this.feeSetItem.other_condition[a]=item[a];
                            };
                        };
